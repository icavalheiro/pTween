...
{
	// public static float Lerp(this float from, float to, float t, AnimationEase ease = AnimationEase.LINEAR){
	// 	return Tween<float>.GetTransaction(from, to, t, ease);
	// }

	public static Tween<float> GetLerp(this float from, float to, AnimationCurve curve){
		var delta = to - from;
		return new Tween<float> ((t) => {
			return from + (delta * curve.Evaluate(t));
		});
	}
}
...
public sealed class Tween<T>{
	...
	private Func<float, T> _evaluteMethod;
	public Tween(Func<float, T> evaluationMethod){
		_evaluteMethod = evaluationMethod;
	}

	public T Evaluate(float normalizedTime){
		return _evaluteMethod(normalizedTime);
	}

	public IEnumerator EnumerateDuring(float seconds, Action<T> onUpdateCallback){
		seconds = Mathf.Abs(seconds);
		float counter = 0;
		while(counter < seconds){
			onUpdateCallback(Evaluate(counter/seconds));
			yield return null;
	...
	// 		yield return null;
	// 		counter += Time.deltaTime;
	// 		callback(0f.Lerp(1f, counter/seconds, ease));
	// 	}
	// }
	...
	// 		yield return null;
	// 		counter += Time.deltaTime;
	// 		callback(0f.Lerp(1f, counter/seconds, curve));
	// 	}
	// }
    ...
    // #region Math stuff
	// public static float GetTransaction(float p_start, float p_end, float p_normalizedTime, AnimationEase p_ease)
	// {
	// 	switch(p_ease)
	// 	{
	// 	default:
